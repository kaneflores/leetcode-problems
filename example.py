...
##################solution 2####################################
    def isAnagram(self, s: str, t: str) -> bool:
        if len(s) != len(t):
            return False
        #create hashmap to count letters from both
        countS, countT = {}, {}

        # fill up hashmaps by adding the letters and incremental values into the hashmap
...
class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        while l<=r:
            ...
            if nums[m] > target:
                #target is on right half portion of the nums list
                r = m-1
            elif nums[m] < target:
                #target is on left portion of the list
                l = m+1
            else:# nums[m] == target
                return m
